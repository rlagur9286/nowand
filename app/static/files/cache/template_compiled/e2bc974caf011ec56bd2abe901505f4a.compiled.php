...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/rss/tpl/rss_admin_index/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
...
<section class="section">
	...
	<?php Context::addJsFile("modules/rss/ruleset/insertRssConfig.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" enctype="multipart/form-data" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="insertRssConfig" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls" style="padding-top:5px"><a href="<?php echo escape($__Context->total_config->url, false) ?>" target="_blank"><?php echo escape($__Context->total_config->url, false) ?></a></div>
		</div>
		<div class="x_control-group">
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<textarea name="feed_description" id="feed_description" rows="4" cols="42" style="float:left;margin-right:8px"><?php echo escape($__Context->total_config->feed_description, false) ?></textarea>
		...
		<div class="x_control-group">
			...
			<div class="_rss_image_container">
				<?php if($__Context->total_config->image){ ?><div class="x_thumbnail" style="display:inline-block;margin:0 0 5px 0"> 
					<img src="/<?php echo escape($__Context->total_config->image, false) ?>" alt="image" style="max-width:210px;max-height:150px" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls">
				<input type="number" min="1" max="100" name="feed_document_count" value="<?php echo escape($__Context->total_config->feed_document_count, false) ?>" id="feed_document_count" />
			</div>
		</div>
...
<section class="section">
	...
	<form action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		...
		<input type="hidden" name="success_return_url" value="<?php echo escape(getUrl('', 'module', $__Context->module, 'act', 'dispRssAdminIndex'), false) ?>" />
		<input type="hidden" name="xe_validator_id" value="modules/rss/tpl/rss_admin_index/1" />
			...
			<tbody>
				<?php if($__Context->feed_config&&count($__Context->feed_config))foreach($__Context->feed_config as $__Context->key=>$__Context->value){ ?><tr>
					<?php if(!$__Context->value['url']){ ?><th>
						<?php echo escape($__Context->value['mid'], false) ?>
					</th><?php } ?>
					<?php if($__Context->value['url']){ ?><th>
						<a href="<?php echo escape($__Context->value['url'], false) ?>" target="_blank"><?php echo escape($__Context->value['mid'], false) ?></a>
					</th><?php } ?>
					<td class="title"><?php echo escape($__Context->value['feed_description'], false) ?><textarea name="feed_description[<?php echo escape($__Context->key, false) ?>]" hidden><?php echo escape($__Context->value['feed_description'], false) ?></textarea></td>
					<td>
						<?php if($__Context->lang->open_rss_types&&count($__Context->lang->open_rss_types))foreach($__Context->lang->open_rss_types as $__Context->key2=>$__Context->value2){ ?><label class="x_inline"><input type="radio" name="open_rss[<?php echo escape($__Context->key, false) ?>]" value="<?php echo escape($__Context->key2, false) ?>"<?php if($__Context->key2 == $__Context->value['open_feed']){ ?> checked="checked"<?php } ?> /> <?php echo escape($__Context->value2, false) ?></label><?php } ?>
					</td>
					<td>
						<label class="x_inline"><input type="radio" name="open_total_feed[<?php echo escape($__Context->key, false) ?>]" value="N"<?php if($__Context->value['open_total_feed'] != 'T_N'){ ?> checked="checked"<?php } ?> /> <?php echo $__Context->lang->use ?></label>
						<label class="x_inline"><input type="radio" name="open_total_feed[<?php echo escape($__Context->key, false) ?>]" value="T_N"<?php if($__Context->value['open_total_feed'] == 'T_N'){ ?> checked="checked"<?php } ?> /> <?php echo $__Context->lang->notuse ?></label>
					</td>
				</tr><?php } ?>
