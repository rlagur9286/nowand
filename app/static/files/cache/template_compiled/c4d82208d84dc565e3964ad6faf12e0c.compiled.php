...
<fieldset class="list">
	<?php if($__Context->lang_code_list&&count($__Context->lang_code_list))foreach($__Context->lang_code_list as $__Context->no=>$__Context->val){ ?><form action="" class="item"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		<a class="set" href="#lang-<?php echo escape($__Context->no, false) ?>" data-toggle data-lang_code="<?php echo escape($__Context->val->name, false) ?>"><span><?php echo escape($__Context->val->value, false) ?></span></a>
		<fieldset id="lang-<?php echo escape($__Context->no, false) ?>" style="display:none">
			<?php if($__Context->lang_supported&&count($__Context->lang_supported))foreach($__Context->lang_supported as $__Context->code=>$__Context->lname){ ?><textarea disabled class="<?php echo escape($__Context->code, false) ?>" data-lang="<?php echo escape($__Context->code, false) ?>" rows="1" cols="12" title="<?php echo escape($__Context->lname, false) ?>" style="margin-right:5px"></textarea><?php } ?>
...
<div class="x_clearfix">
	<?php if($__Context->page_navigation){ ?><form action="./" class="x_pagination x_pull-left" data-search_keyword="<?php echo htmlspecialchars($__Context->search_keyword, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" data-page="<?php echo escape($__Context->page, false) ?>" data-current_lang="<?php echo escape($__Context->lang_code, false) ?>" ><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		<ul>
			...
			<li<?php if($__Context->page_no == $__Context->page){ ?> class="x_active"<?php } ?>><a  href="#" data-page="<?php echo escape($__Context->page_no, false) ?>" data-search_keyword="<?php echo htmlspecialchars($__Context->search_keyword, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>"><?php echo escape($__Context->page_no, false) ?></a></li>
			<?php } ?>
			...
			<?php } ?>
		<li<?php if($__Context->page == $__Context->page_navigation->last_page){ ?> class="x_disabled"<?php } ?>><a href="#" data-page="<?php echo escape($__Context->page_navigation->last_page, false) ?>" data-search_keyword="<?php echo htmlspecialchars($__Context->search_keyword, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" title="<?php echo escape($__Context->page_navigation->last_page, false) ?>"><?php echo $__Context->lang->last_page ?> &raquo;</a></li>
		</ul>
	</form><?php } ?>
	<form action="" class="search center x_input-append x_pull-right"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		<select name="lang_code" style="margin-right:4px">
			<?php if($__Context->lang_supported&&count($__Context->lang_supported))foreach($__Context->lang_supported as $__Context->code=>$__Context->lname){ ?><option value="<?php echo escape($__Context->code, false) ?>"<?php if($__Context->code == $__Context->lang_code){ ?> selected="selected"<?php } ?>><?php echo escape($__Context->lname, false) ?></option><?php } ?>
		</select>
