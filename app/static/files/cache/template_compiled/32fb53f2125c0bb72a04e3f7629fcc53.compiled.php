...
<form action="./index.php" method="get" name="calendar" id="fo_calendar" style="margin-bottom:30px"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
    ...
    <input type="hidden" name="selected_date" id="selected_date" value="<?php echo escape(zdate($__Context->selected_date,'Ymd'), false) ?>" />
	<table class="x_table x_table-striped x_table-hover">
		<thead>
			<tr>
				<th scope="row"><?php echo $__Context->lang->total_counter ?></th>
				<th scope="row"> 
					<span id="str_selected_date"><?php echo escape(zdate($__Context->selected_date, "Y-m-d"), false) ?></span>
					<input type="hidden" class="inputDate" value="<?php echo escape(zdate($__Context->selected_date,'Y-m-d'), false) ?>" />
					<script>
					(function($){
						$(function(){
							var option = {
		...
		<tbody>
			<tr>
				<td>Visitor <strong><?php echo escape(number_format($__Context->total_counter->unique_visitor), false) ?></strong> &nbsp;/&nbsp; Pageview : <strong><?php echo escape(number_format($__Context->total_counter->pageview), false) ?></strong></td>
				<td>Visitor : <strong><?php echo escape(number_format($__Context->selected_day_counter->unique_visitor), false) ?></strong> &nbsp;/&nbsp; Pageview : <strong><?php echo escape(number_format($__Context->selected_day_counter->pageview), false) ?></strong></td>
			</tr>
		</tbody>
	</table>
</form>
<ul class="x_nav x_nav-tabs">
	<?php if($__Context->lang->cmd_select_counter_type&&count($__Context->lang->cmd_select_counter_type))foreach($__Context->lang->cmd_select_counter_type as $__Context->key=>$__Context->val){ ?><li<?php if($__Context->type==$__Context->key || (!$__Context->type && $__Context->key == 'hour')){ ?> class="x_active"<?php } ?>>
		<a href="<?php echo escape(getUrl('type',$__Context->key), false) ?>"><?php echo escape($__Context->val, false) ?></a> 
	</li><?php } ?>
</ul>
<!-- 일자를 옮기는 form -->
<form action="./" method="get" id="fo_counter"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	<input type="hidden" name="module" value="<?php echo escape($__Context->module, false) ?>" />
	<input type="hidden" name="act" value="<?php echo escape($__Context->act, false) ?>" />
	<input type="hidden" name="selected_date" value="<?php echo escape($__Context->selected_date, false) ?>" />
</form>
<!-- unique visitor 그래프 -->
<table class="x_table x_table-striped x_table-hover">
	...
	<tr>
		<?php if(Context::getLangType()=='en'){ ?><th scope="row"> 
			<?php if($__Context->type == 'year'){ ?>
				<?php echo escape($__Context->key, false) ?>
			<?php }elseif($__Context->type == 'month'){ ?>
				<?php echo escape($__Context->key, false) ?>
			<?php }elseif($__Context->type == 'day'){ ?>
				<?php echo escape($__Context->key, false) ?> <?php echo $__Context->lang->unit_day ?>
			<?php }else{ ?>
				<?php echo escape($__Context->key, false) ?>
			<?php } ?>
		</th><?php } ?>
		<?php if(Context::getLangType()!='en'){ ?><th scope="row"> 
			<?php if($__Context->type == 'year'){ ?>
				<?php echo escape($__Context->key, false) ?> <?php echo $__Context->lang->unit_year ?>
			<?php }elseif($__Context->type == 'month'){ ?>
				<?php echo escape($__Context->key, false) ?> <?php echo $__Context->lang->unit_month ?>
			<?php }elseif($__Context->type == 'day'){ ?>
				<?php echo escape($__Context->key, false) ?> <?php echo $__Context->lang->unit_day ?>
			<?php }else{ ?>
				<?php echo escape($__Context->key, false) ?> <?php echo $__Context->lang->unit_hour ?>
			<?php } ?>
		</th><?php } ?>
		<?php  $__Context->img_width = (int)$__Context->percent; $__Context->img_width = $__Context->img_width?$__Context->img_width.'%':'3px';  ?>
		<td>
			<div class="barContainer">
				<span class="graphHr" style="width:<?php echo escape($__Context->img_width, false) ?>">
					<span><?php echo escape(number_format($__Context->val), false) ?>(<?php echo escape($__Context->percent, false) ?>%)</span>
				</span>
			</div>
		</td>
	</tr>
