...
var layoutTitle = '<?php echo escape($__Context->layout->layout_title, false) ?>';
...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/layout/tpl/copy_layout/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
...
<form action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<div class="x_modal-body x_form-horizontal" id="inputDiv">
		<input type="hidden" name="layout" value="<?php echo escape($__Context->layout->layout, false) ?>" />
		...
		<div class="x_control-group">
			...
			<div class="x_controls"><?php echo escape($__Context->layout->title, false) ?></div>
		</div>
		<div class="x_control-group">
			<div class="x_control-label"><?php echo $__Context->lang->title ?></div>
			<div class="x_controls"><?php echo escape($__Context->layout->layout_title, false) ?></div>
		</div>
		<div class="x_control-group">
			<label class="x_control-label" for=""><?php echo $__Context->lang->new_title ?></label>
			<div class="x_controls">
				<span class="x_input-append">
					<input type="text" name="title[]" required placeholder="<?php echo escape($__Context->layout->layout_title, false) ?>" />
