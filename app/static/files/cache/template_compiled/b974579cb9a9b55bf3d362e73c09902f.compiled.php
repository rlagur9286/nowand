...
<form id="fo_list" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/file/tpl/file_list/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
	...
	<table id="fileListTable" class="x_table x_table-striped x_table-hover">
		<caption>
			<a href="<?php echo escape(getUrl('', 'module', 'admin', 'act', 'dispFileAdminList'), false) ?>"<?php if($__Context->search_keyword == ''){ ?> class="active"<?php } ?>><?php echo $__Context->lang->all;
if($__Context->search_keyword == ''){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('search_target','isvalid','search_keyword','Y'), false) ?>"<?php if($__Context->search_target == 'isvalid' && $__Context->search_keyword == 'Y'){ ?> class="active"<?php } ?>><?php echo $__Context->lang->is_valid;
if($__Context->search_target == 'isvalid' && $__Context->search_keyword == 'Y'){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('search_target','isvalid','search_keyword','N'), false) ?>"<?php if($__Context->search_target == 'isvalid' && $__Context->search_keyword == 'N'){ ?> class="active"<?php } ?>><?php echo $__Context->lang->is_stand_by;
if($__Context->search_target == 'isvalid' && $__Context->search_keyword == 'N'){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
				...
				<th colspan="8" scope="col">
					...
					<?php }else{ ?>
						...
						<?php if($__Context->document_list[$__Context->document_srl] && ($__Context->val->upload_target_type == 'doc' || $__Context->val->upload_target_type == 'com')){ ?>
						 - <?php if($__Context->document_list[$__Context->document_srl]->get('module_srl') != $__Context->document_list[$__Context->document_srl]->get('member_srl')){ ?><a href="<?php echo escape($__Context->move_uri, false) ?>" target="_blank"><?php echo $__Context->document_list[$__Context->document_srl]->getTitle() ?></a><?php }else{;
echo $__Context->document_list[$__Context->document_srl]->getTitle();
			...
			<tr>
				<td>	<?php if(preg_match("/\.(jpg|png|bmp|jpeg|gif)$/i",strtolower($__Context->val->source_filename))){ ?>
					<img src="<?php echo escape($__Context->val->uploaded_filename, false) ?>" style="width:50px; height:50px">
					<?php } ?>
					<a href="<?php echo escape(htmlspecialchars_decode($__Context->val->download_url), false) ?>"><?php echo htmlspecialchars($__Context->val->source_filename, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?></a></td>
				<td class="nowr"><?php echo escape(FileHandler::filesize($__Context->val->file_size), false) ?></td>
				<td class="nowr"><?php echo escape($__Context->val->download_count, false) ?></td>
				<td class="nowr">
					<?php if($__Context->val->upload_target_type == 'doc' && $__Context->document_list[$__Context->document_srl]){ ?>
					<a href="#popup_menu_area" class="member_<?php echo escape($__Context->document_list[$__Context->document_srl]->get('member_srl'), false) ?>"><?php echo escape($__Context->document_list[$__Context->document_srl]->getNickName(), false) ?></a>
					<?php }elseif($__Context->val->upload_target_type == 'com' && $__Context->comment_list[$__Context->val->upload_target_srl]){ ?>
					<a href="#popup_menu_area" class="member_<?php echo escape($__Context->comment_list[$__Context->val->upload_target_srl]->get('member_srl'), false) ?>"><?php echo escape($__Context->comment_list[$__Context->val->upload_target_srl]->getNickName(), false) ?></a>
					<?php }else{ ?>
					<a href="#popup_menu_area" class="member_<?php echo escape($__Context->val->member_srl, false) ?>"><?php echo escape($__Context->val->nick_name, false) ?></a>
					<?php } ?>
				</td>
				<td class="nowr"><?php echo escape(zdate($__Context->val->regdate,"Y-m-d H:i"), false) ?></td>
...
echo $__Context->lang->is_stand_by;
				...
				<td>
					<input type="checkbox" name="cart" value="<?php echo escape($__Context->val->file_srl, false) ?>" />
				</td>
			</tr>
			<?php } ?>
